...
# 检查版本号格式（支持 beta、alpha、rc 等预发布版本）
if ($Version -notmatch '^v\d+\.\d+\.\d+(-[a-zA-Z0-9.-]+)?$') {
    Write-Host "错误: 版本号格式不正确" -ForegroundColor Red
    Write-Host "格式: vMAJOR.MINOR.PATCH[-PRERELEASE]" -ForegroundColor Yellow
    Write-Host "示例: v1.0.0, v1.1.0, v1.0.1, v0.1.0-beta.1, v1.0.0-rc.1" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "  go get github.com/FastTLS/fastls@$Version"
